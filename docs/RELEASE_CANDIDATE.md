...
## Hallazgos de esta auditoría (riesgos residuales de código, no bloqueantes para el piloto actual)

Durante la ejecución fresca de ROT-02/ROT-06/CAS-02 se encontraron dos comportamientos de borde que vale la pena que un humano revise antes de operar con más de un pool de rotación activo simultáneamente por grupo:

1. **El rechazo/expiración de una oferta con `rejectionConsumesTurn=true` reinicia el estado de fila de *todos* los miembros del pool a `AVAILABLE`, no sólo el de quien rechazó.** Código: `rejectRotationCandidate` en `apps/api-worker/src/services/rotation-response-service.ts`, el bucle `for(const item of after) ... status='AVAILABLE'`. Efecto observado: un empleado que ya estaba `ASSIGNED` (cubriendo activamente ese mismo pool en otro expediente con fechas no traslapadas) quedó momentáneamente disponible para ser ofertado de nuevo en el pool, cuando debería seguir marcado como ocupado hasta que su cobertura real termine. Se corrigió manualmente el dato de producción afectado (ver detalle en el reporte de esta tarea) porque la prueba lo causó; el código no se modificó (fuera de alcance de esta auditoría). Recomendación: acotar ese `UPDATE` a los empleados cuyo estado antes del rechazo era `RESERVED`/`AVAILABLE`, no a todo el pool.
2. **`cancelCoverage` toma `assignments[0]` sin filtrar por estado activo cuando un expediente tiene más de una fila en `temporary_assignments`** (por ejemplo tras una cascada de oferta vencida: una fila `CANCELLED` y otra `PROPOSED`). En una prueba, esto hizo que el paso de liberar la fila de la cola (`rotation_queue_entries.status='AVAILABLE'`) se aplicara al empleado equivocado. Recomendación: filtrar por `status NOT IN ('CANCELLED','COMPLETED','REPLACED')` antes de tomar `[0]`, o iterar sobre todas las asignaciones no terminales.

Ninguno de los dos afecta las reglas de negocio centrales (nivel base inmutable, transición autorizada, auditoría) — son inconsistencias de estado derivado en `rotation_queue_entries.status`, recuperables manualmente y detectables por auditoría. Se documentan aquí para que no se pierdan antes de operar con datos reales de más empleados.
