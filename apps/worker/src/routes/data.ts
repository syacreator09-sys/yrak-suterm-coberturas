...
dataRoutes.get('/exports/employees.csv', requireRoles('ADMIN', 'HR', 'AUDITOR'), async (context) => {
  const result = await context.env.DB.prepare(`SELECT employee_number AS employeeNumber,
...
dataRoutes.get('/exports/coverages.csv', requireRoles('ADMIN', 'HR', 'AUDITOR'), async (context) => {
  const result = await context.env.DB.prepare(`SELECT cc.folio, e.employee_number AS absentEmployeeNumber,
  ...
  return context.body(
    toCsv(
      ['folio','absentEmployeeNumber','absentEmployee','startsAt','endsAt','durationDays','processType','status','reason','ruleVersion'],
      result.results ?? [],
    ),
    200,
    {
...
dataRoutes.post('/imports/employees/preview', requireRoles('ADMIN', 'HR'), async (context) => {
  ...
  const existing = await context.env.DB.prepare('SELECT employee_number FROM employees WHERE organization_id = ?')
    .bind(organizationId)
  ...
  const rows = parsed.map((row, index) => {
    ...
    if (fileNumbers.has(employeeNumber)) errors.push('employeeNumber: duplicado dentro del archivo');
...
dataRoutes.post(
  ...
  async (context) => {
    ...
    const statements = rows.map((row) =>
      context.env.DB.prepare(`INSERT INTO employees (
