...
dashboardRoutes.get('/dashboard', async (context) => {
  ...
  const [active, competitions, requirements, pendingApprovals] = await context.env.DB.batch([
    context.env.DB.prepare(
      "SELECT COUNT(*) AS total FROM coverage_cases WHERE group_id IN (SELECT id FROM groups WHERE organization_id = ?) AND status IN ('SCHEDULED','ACTIVE')",
    ).bind(user.organizationId),
    context.env.DB.prepare(
      "SELECT COUNT(*) AS total FROM competitions c JOIN coverage_cases cc ON cc.id = c.coverage_case_id JOIN groups g ON g.id = cc.group_id WHERE g.organization_id = ? AND c.status NOT IN ('FINAL','CANCELLED')",
    ).bind(user.organizationId),
    context.env.DB.prepare(
      "SELECT COUNT(*) AS total FROM employee_requirements er JOIN employees e ON e.id = er.employee_id WHERE e.organization_id = ? AND er.status IN ('MISSING','EXPIRED','PENDING','REJECTED')",
    ).bind(user.organizationId),
    context.env.DB.prepare(
