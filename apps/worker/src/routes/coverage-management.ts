...
export const coverageManagementRoutes = new Hono<AppBindings>();
coverageManagementRoutes.use('*', authenticate);

coverageManagementRoutes.get(
  '/coverages',
  ...
  async (context) => {
    ...
    const result = isOrganizationWide(user.roles)
      ? await context.env.DB.prepare(`SELECT cc.*, a.reason,
...
coverageManagementRoutes.get(
  ...
  async (context) => {
    const user = context.get('user');
    const item = await context.env.DB.prepare(`SELECT cc.*,
    ...
    const employeeOnly =
      user.roles.includes('EMPLOYEE') && !isOrganizationWide(user.roles);
    if (employeeOnly) {
    ...
    const [assignments, approvals, countedDays, audit] = await context.env.DB.batch([
      context.env.DB.prepare(`SELECT * FROM temporary_assignments
        WHERE coverage_case_id = ? ORDER BY chain_order`)
        .bind(context.req.param('id')),
      context.env.DB.prepare(`SELECT * FROM approvals
        WHERE entity_type = 'COVERAGE_CASE' AND entity_id = ? ORDER BY requested_at`)
        .bind(context.req.param('id')),
      context.env.DB.prepare(`SELECT counted_date, counting_mode, source
        FROM coverage_counted_days WHERE coverage_case_id = ? ORDER BY counted_date`)
        .bind(context.req.param('id')),
      context.env.DB.prepare(`SELECT actor_id, actor_type, action, rule_applied,
          reason, occurred_at
        FROM audit_events
...
coverageManagementRoutes.post(
  ...
  async (context) => {
    const approval = await context.env.DB.prepare(`SELECT a.entity_id
      FROM approvals a
