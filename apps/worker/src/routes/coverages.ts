...
coverageRoutes.get(
  ...
  async (context) => {
    ...
    const privileged = user.roles.some((role) =>
      ['ADMIN', 'HR', 'AUDITOR', 'COMMITTEE'].includes(role),
    );
    if (!privileged && user.groups.length === 0) return context.json({ items: [] });
    const result = privileged
      ? await context.env.DB.prepare(
          `SELECT cc.*, a.reason, e.name AS absent_employee_name FROM coverage_cases cc JOIN absences a ON a.id = cc.absence_id JOIN employees e ON e.id = a.employee_id JOIN groups g ON g.id = cc.group_id WHERE g.organization_id = ? ORDER BY cc.created_at DESC LIMIT 200`,
        )
          .bind(user.organizationId)
          .all()
      : await context.env.DB.prepare(
          `SELECT cc.*, a.reason, e.name AS absent_employee_name FROM coverage_cases cc JOIN absences a ON a.id = cc.absence_id JOIN employees e ON e.id = a.employee_id JOIN groups g ON g.id = cc.group_id WHERE g.organization_id = ? AND cc.group_id IN (${user.groups.map(() => '?').join(',')}) ORDER BY cc.created_at DESC LIMIT 200`,
        )
...
coverageRoutes.get(
  ...
  async (context) => {
    ...
    const item = await context.env.DB.prepare(
      `SELECT cc.*, a.employee_id AS absent_employee_id, a.reason, e.name AS absent_employee_name FROM coverage_cases cc JOIN absences a ON a.id = cc.absence_id JOIN employees e ON e.id = a.employee_id JOIN groups g ON g.id = cc.group_id WHERE cc.id = ? AND g.organization_id = ?`,
    )
    ...
    const staffRole = user.roles.some((role) =>
    ...
    if (user.roles.includes('EMPLOYEE') && !staffRole) {
      const related = await context.env.DB.prepare(
        'SELECT 1 AS related FROM temporary_assignments WHERE coverage_case_id = ? AND employee_id = ? LIMIT 1',
      )
    ...
    const assignments = await context.env.DB.prepare(
      'SELECT * FROM temporary_assignments WHERE coverage_case_id = ? ORDER BY chain_order',
    )
      .bind(context.req.param('id'))
      .all();
    const approvals = await context.env.DB.prepare(
...
coverageRoutes.post(
  ...
  requireIdempotency('CREATE_COVERAGE'),
  zValidator(
    'json',
    z.object({
      ...
      competition: z
        .object({
          ...
          tieBreakers: z
            .array(z.object({ type: z.enum(['CRITICAL_SECTION', 'SENIORITY', 'EMPLOYEE_ID']) }))
            .optional(),
        })
        .optional(),
    }),
