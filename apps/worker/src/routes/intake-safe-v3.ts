...
async function claimDraft(context: Context<AppBindings>, draftId: string): Promise<boolean> {
  try {
...
export const safeIntakeRoutesV3 = new Hono<AppBindings>();
safeIntakeRoutesV3.use('*', authenticate);

safeIntakeRoutesV3.get(
  '/intake-drafts',
...
safeIntakeRoutesV3.post(
  '/intake-drafts/:id/approve',
  requireRoles('ADMIN', 'HR', 'SUPERVISOR', 'OPERATOR'),
  requireIdempotency('APPROVE_SAFE_INTAKE_DRAFT_V3'),
  zValidator(
    'json',
    z.object({
...
safeIntakeRoutesV3.post(
  ...
  requireRoles('ADMIN', 'HR', 'SUPERVISOR', 'OPERATOR'),
  requireIdempotency('REJECT_SAFE_INTAKE_DRAFT_V3'),
  async (context) => {
    const user = context.get('user');
    const draftId = context.req.param('id');
