...
uploadRoutes.post(
  ...
  async (context) => {
    ...
    if (!allowedMimeTypes.has(file.type))
      return context.json({ error: 'UNSUPPORTED_MIME_TYPE' }, 415);
    if (file.size > maximumBytes)
      return context.json({ error: 'FILE_TOO_LARGE', maximumBytes }, 413);

    const user = context.get('user');
    const bytes = await file.arrayBuffer();
    if (!matchesMagic(file.type, bytes))
      return context.json({ error: 'MIME_SIGNATURE_MISMATCH' }, 415);
