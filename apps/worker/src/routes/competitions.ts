...
competitionRoutes.get(
  ...
  async (context) => {
    ...
    const candidates = await context.env.DB.prepare(
      `SELECT cc.*, e.name, e.employee_number
...
competitionRoutes.post(
  ...
  async (context) => {
    ...
    if (
      ...
      !user.roles.some((role) => ['ADMIN', 'HR'].includes(role)) &&
...
competitionRoutes.post(
  ...
  async (context) => {
    const result = await recordScore(
      ...
      { competitionId: context.req.param('id'), ...context.req.valid('json') },
    );
