...
const RoleSchema = z.enum(['ADMIN','HR','SUPERVISOR','COMMITTEE','OPERATOR','EMPLOYEE','AUDITOR']);
...
userRoutes.get('/users', async (context) => {
  const result = await context.env.DB.prepare(`SELECT u.id, u.employee_id, u.external_subject,
  ...
  async (context) => {
    ...
    const statements: D1PreparedStatement[] = [
      context.env.DB.prepare(`INSERT INTO app_users (
        ...
        .bind(id, actor.organizationId, input.employeeId ?? null, input.externalSubject, input.email, input.displayName),
    ];
    for (const role of input.roles) {
      if (['SUPERVISOR','OPERATOR'].includes(role) && input.groupIds.length > 0) {
        for (const groupId of input.groupIds) {
          statements.push(
            context.env.DB.prepare('INSERT INTO user_roles (user_id, role, group_id) VALUES (?, ?, ?)')
              .bind(id, role, groupId),
          );
        }
      } else {
        statements.push(
          context.env.DB.prepare('INSERT INTO user_roles (user_id, role, group_id) VALUES (?, ?, NULL)')
            .bind(id, role),
        );
      }
    }
