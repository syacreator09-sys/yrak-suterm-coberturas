...
appealRoutes.get(
  ...
  async (context) => {
    ...
    const privileged = user.roles.some((role) => ['ADMIN', 'HR', 'COMMITTEE', 'AUDITOR'].includes(role));
    const result = privileged
      ? await context.env.DB.prepare(`SELECT a.*, cc.folio, e.name AS employee_name
...
appealRoutes.post(
  ...
  async (context) => {
    ...
    const competition = await context.env.DB.prepare(`SELECT c.id, c.coverage_case_id,
    ...
    await context.env.DB.batch([
      context.env.DB.prepare(`INSERT INTO appeal_cases (
      ...
      context.env.DB.prepare(`INSERT INTO appeal_events (
        id, appeal_id, actor_id, action, reason
      ) VALUES (?, ?, ?, 'SUBMITTED', ?)`)
        .bind(crypto.randomUUID(), id, user.id, input.reason),
      context.env.DB.prepare(`UPDATE coverage_cases
...
appealRoutes.post(
  ...
  async (context) => {
    const user = context.get('user');
    const result = await context.env.DB.prepare(`UPDATE appeal_cases
    ...
    await context.env.DB.prepare(`INSERT INTO appeal_events (
...
appealRoutes.post(
  ...
  async (context) => {
    const user = context.get('user');
    const input = context.req.valid('json');
    const appeal = await context.env.DB.prepare(`SELECT id, submitted_by, coverage_case_id
    ...
    await context.env.DB.batch([
      context.env.DB.prepare(`UPDATE appeal_cases SET
