...
intakeRoutes.post(
  ...
  async (context) => {
    ...
    const draft = await context.env.DB.prepare(
      "SELECT id FROM intake_drafts WHERE id = ? AND organization_id = ? AND status = 'REVIEW_PENDING'",
    )
    ...
    const result = await createCoverage(
      ...
      { ...context.req.valid('json'), source: 'INTEGRATION' },
    );
    await context.env.DB.prepare(`UPDATE intake_drafts
      ...
      .bind(String(result.caseId), user.id, draft.id)
      .run();
