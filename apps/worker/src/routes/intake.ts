...
intakeRoutes.get('/intake-drafts', requireRoles('ADMIN', 'HR', 'SUPERVISOR', 'OPERATOR'), async (context) => {
  const result = await context.env.DB.prepare(`SELECT id, attachment_id, source_type,
      draft_json, status, linked_coverage_case_id, created_at, updated_at
    FROM intake_drafts WHERE organization_id = ? ORDER BY created_at DESC LIMIT 200`)
    .bind(context.get('user').organizationId).all();
...
intakeRoutes.post(
  ...
  requireIdempotency('APPROVE_INTAKE_DRAFT'),
  ...
  async (context) => {
    ...
    const draft = await context.env.DB.prepare("SELECT id FROM intake_drafts WHERE id = ? AND organization_id = ? AND status = 'REVIEW_PENDING'")
      .bind(context.req.param('id'), user.organizationId).first<{ id: string }>();
    if (!draft) return context.json({ error: 'REVIEW_PENDING_DRAFT_NOT_FOUND' }, 404);
    const result = await createCoverage(context.env, user, context.get('correlationId'), { ...context.req.valid('json'), source: 'INTEGRATION' });
    await context.env.DB.prepare(`UPDATE intake_drafts
      ...
      .bind(String(result.caseId), user.id, draft.id).run();
    return context.json({ draftId: draft.id, coverage: result }, 201);
  },
);

intakeRoutes.post('/intake-drafts/:id/reject', requireRoles('ADMIN','HR','SUPERVISOR','OPERATOR'), requireIdempotency('REJECT_INTAKE_DRAFT'), async (context) => {
  const user = context.get('user');
  await context.env.DB.prepare(`UPDATE intake_drafts
    ...
    .bind(user.id, context.req.param('id'), user.organizationId).run();
