...
channelRoutes.get('/email-channels', async (context) => {
  const result = await context.env.DB.prepare(`SELECT id, inbound_address, active, created_at
...
channelRoutes.post(
  ...
  zValidator('json', z.object({ inboundAddress: z.email().transform((value) => value.toLowerCase()) })),
  async (context) => {
    const id = crypto.randomUUID();
    const address = context.req.valid('json').inboundAddress;
    await context.env.DB.prepare(`INSERT INTO organization_email_channels (
