...
export async function cancelCoverage(
  ...
  const coverage = await env.DB.prepare(
    `SELECT cc.id, cc.absence_id, cc.group_id,
  ...
  if (coverage.status === 'COMPLETED') {
    throw new DomainError(
      'COMPLETED_COVERAGE_IMMUTABLE',
      'Una cobertura terminada no puede cancelarse',
    );
  }
  if (coverage.status === 'CANCELLED') return { coverageCaseId: coverage.id, status: 'CANCELLED' };

  const assignments = await env.DB.prepare(
    `SELECT id, employee_id, base_level_id,
  ...
  const statements: D1PreparedStatement[] = [
    ...
    env.DB.prepare(
      "UPDATE temporary_assignments SET status = 'CANCELLED', version = version + 1, updated_at = datetime('now') WHERE coverage_case_id = ? AND status NOT IN ('COMPLETED','CANCELLED','REPLACED')",
    ).bind(coverage.id),
    env.DB.prepare(
      "UPDATE approvals SET status = 'CANCELLED', decided_by = ?, decided_at = datetime('now'), reason = ? WHERE entity_type = 'COVERAGE_CASE' AND entity_id = ? AND status = 'PENDING'",
    ).bind(user.id, input.reason, coverage.id),
    env.DB.prepare(
      "UPDATE competitions SET status = 'CANCELLED', version = version + 1, updated_at = datetime('now') WHERE coverage_case_id = ? AND status <> 'FINAL'",
    ).bind(coverage.id),
  ];
  if (input.cancelAbsence)
    statements.push(
      env.DB.prepare("UPDATE absences SET status = 'CANCELLED' WHERE id = ?").bind(
        coverage.absence_id,
      ),
    );
  if (coverage.process_type === 'ROTATION') {
    for (const assignment of assignments.results ?? []) {
      ...
      const entry = await env.DB.prepare(
        'SELECT queue_position FROM rotation_queue_entries WHERE pool_id = ? AND employee_id = ?',
      )
        .bind(pool.id, assignment.employee_id)
        .first<{ queue_position: number }>();
      statements.push(
        env.DB.prepare(
          "UPDATE rotation_queue_entries SET availability = 'AVAILABLE', version = version + 1, updated_at = datetime('now') WHERE pool_id = ? AND employee_id = ? AND availability IN ('RESERVED','ASSIGNED')",
        ).bind(pool.id, assignment.employee_id),
      );
      if (entry)
        statements.push(
