...
async function createRotationAssignment(
  ...
  while (candidates.some((candidate) => candidate.availability === 'AVAILABLE')) {
    ...
    const reservation = await env.GROUP_COORDINATOR.getByName(groupId).reserve({
  ...
  if (!selected)
    throw new DomainError('NO_AVAILABLE_ROTATION_CANDIDATE', 'No hay candidatos disponibles');
  const assignmentId = crypto.randomUUID();
  const approvalId = crypto.randomUUID();
  await env.DB.batch([
    env.DB.prepare(
      "INSERT INTO temporary_assignments (id, coverage_case_id, employee_id, base_level_id, target_level_id, chain_order, starts_at, ends_at, status) VALUES (?, ?, ?, ?, ?, 1, ?, ?, 'PROPOSED')",
    ).bind(
      assignmentId,
      caseId,
      selected.employeeId,
    ...
    env.DB.prepare(
      "INSERT INTO rotation_events (id, pool_id, employee_id, coverage_case_id, event_type, previous_position, new_position, reason, created_by) SELECT ?, ?, employee_id, ?, 'RESERVED', queue_position, queue_position, 'Candidato reservado para aprobación', ? FROM rotation_queue_entries WHERE pool_id = ? AND employee_id = ?",
    ).bind(crypto.randomUUID(), poolId, caseId, user.id, poolId, selected.employeeId),
    env.DB.prepare(
      "INSERT INTO approvals (id, entity_type, entity_id, action, status, requested_by) VALUES (?, 'COVERAGE_CASE', ?, 'APPROVE_ROTATION_ASSIGNMENT', 'PENDING', ?)",
...
async function createCompetition(
  ...
  const mustRemainValid = (requiredResult.results ?? []).some(
  ...
  for (const employee of employeeResult.results ?? []) {
    ...
    candidateStatements.push(
      env.DB.prepare(
        'INSERT INTO competition_candidates (id, competition_id, employee_id, eligibility_status, eligibility_details_json) VALUES (?, ?, ?, ?, ?)',
      ).bind(
        crypto.randomUUID(),
        competitionId,
        employee.id,
  ...
  await env.DB.batch([
    env.DB.prepare(
      "INSERT INTO competitions (id, coverage_case_id, target_level_id, registration_starts_at, registration_ends_at, exam_at, minimum_score, tie_breaker_rules_json, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'OPEN')",
    ).bind(
      competitionId,
      caseId,
      targetLevelId,
...
export async function createCoverage(
  ...
  if (!employee || employee.active !== 1 || employee.organization_id !== user.organizationId)
    throw new DomainError(
      ...
      'No existe la persona ausente en la organización',
    );
  ...
  if (!transition)
    throw new DomainError(
      ...
      'No existe nivel inferior autorizado para cubrir la vacante',
    );
  ...
  await env.DB.batch([
    env.DB.prepare(
      "INSERT INTO absences (id, employee_id, group_id, level_id, reason, starts_at, ends_at, status, source, created_by) VALUES (?, ?, ?, ?, ?, ?, ?, 'CONFIRMED', ?, ?)",
    ).bind(
      absenceId,
      employee.id,
      employee.group_id,
    ...
    env.DB.prepare(
      "INSERT INTO coverage_cases (id, folio, absence_id, group_id, vacant_level_id, starts_at, ends_at, duration_days, process_type, status, rule_version, created_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'PENDING_VALIDATION', 'YR-1.0.0', ?)",
    ).bind(
      caseId,
      folio,
      absenceId,
