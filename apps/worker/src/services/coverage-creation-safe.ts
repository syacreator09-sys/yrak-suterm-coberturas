...
export async function createSafeCoverage(
  ...
  const employee = await env.DB.prepare(`SELECT id, organization_id, group_id,
  ...
  const existingAbsence = await env.DB.prepare(`SELECT id FROM absences
  ...
  const transition = await env.DB.prepare(`SELECT source_level_id, target_level_id
  ...
  await env.DB.batch([
    ...
    ...duration.countedDates.map((date) =>
      env.DB.prepare(`INSERT INTO coverage_counted_days (
  ...
  try {
    ...
    await env.DB.batch([
      env.DB.prepare("UPDATE coverage_cases SET status = 'CANCELLED', updated_at = datetime('now') WHERE id = ?")
        .bind(caseId),
