...
export async function completeCoverage(
  ...
  const coverage = await env.DB.prepare(
    `SELECT cc.id, cc.group_id, cc.process_type,
  ...
  if (coverage.status === 'COMPLETED') {
    const existing = await env.DB.prepare('SELECT actual_ended_at FROM coverage_cases WHERE id = ?')
      .bind(coverage.id)
  ...
  try {
    ...
    if (coverage.process_type === 'ROTATION') {
      for (const assignment of assignments) {
        ...
        const maximum = await env.DB.prepare(
          `SELECT MAX(queue_position) AS maximum
    ...
    await env.DB.prepare(
      ...
      .bind(JSON.stringify({ coverageCaseId: coverage.id, status: 'COMPLETED' }), claimId)
      .run();
    return {
      coverageCaseId: coverage.id,
      status: 'COMPLETED',
