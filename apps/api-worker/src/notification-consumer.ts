...
export async function processNotification(env:AppEnv,notificationId:string):Promise<void>{
  ...
  try{const rendered=renderTemplate(row.template_key,JSON.parse(row.payload_json));await env.EMAIL.send({from:env.EMAIL_FROM,to:row.recipient,subject:rendered.subject,text:rendered.text});await env.DB.prepare(`UPDATE notifications SET status='SENT',sent_at=datetime('now'),last_error=NULL WHERE id=?`).bind(row.id).run();}catch(error){await env.DB.prepare(`UPDATE notifications SET status='FAILED',last_error=? WHERE id=?`).bind(error instanceof Error?error.message:String(error),row.id).run();throw error;}
}
