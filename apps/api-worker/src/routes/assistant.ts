...
assistantRoutes.post('/', requireRoles('ADMIN','HR','SUPERVISOR','COMMITTEE','OPERATOR','EMPLOYEE','AUDITOR'),
  zValidator('json', z.object({ question: z.string().min(1).max(2000) })), async (c) => {
    const u = c.get('user');
    if (!c.env.AGENT_WORKER || !c.env.AGENT_API_TOKEN) return c.json({ success: false, error: { code: 'ASSISTANT_NOT_CONFIGURED', message: 'Asistente no configurado' } }, 503);
    const sessionId = `user-${u.id}`.slice(0, 120);
    const response = await c.env.AGENT_WORKER.fetch(`https://agent-worker/v1/support/${sessionId}`, {
      method: 'POST',
