...
async function persist(c: any, input: z.infer<typeof draftSchema>) {
...
intakeRoutes.post('/extract-text', intakeRoles, zValidator('json', z.object({ content: z.string().min(1) })), async (c) => {
...
intakeRoutes.post('/attachments/:attachmentId/process', intakeRoles, async (c) => {
  ...
  const row = await c.env.DB.prepare(`SELECT id,original_filename,mime_type,r2_key,uploaded_by
    FROM attachments WHERE id=? AND organization_id=?`)
...
intakeRoutes.post('/drafts/:draftId/consume', intakeRoles, zValidator('json', z.object({
  ...
  await c.env.DB.prepare(`UPDATE intake_drafts SET status='CONSUMED',reviewed_by=?,reviewed_at=datetime('now')
