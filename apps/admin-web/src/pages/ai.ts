...
import { canRunSyntheticDiagnostic } from '../core/security.js';
  ...
  try {
    ...
    ctx.root.innerHTML = `<div class="page-stack">
      ...
      <section class="page-grid">
        <article class="panel span-8"><div class="section-heading"><div><h2>Agentes permitidos</h2><p>Capacidades acotadas por diseño.</p></div></div>
          <div class="integration-grid" style="margin-top:12px">${agents.map((agent) => `<article class="integration-card card"><header><h3>${agent.name}</h3>${renderHealth('unknown')}</header><p>${agent.purpose}</p><div class="integration-meta"><div><span>Autoridad laboral</span><strong>Ninguna</strong></div><div><span>Runtime</span><strong>Se verifica al desplegar agent-worker</strong></div></div></article>`).join('')}</div>
        </article>
        <article class="panel span-4"><h2>Runtime de IA</h2><div class="key-value"><dt>Workers AI binding</dt><dd>${health.workersAi}</dd><dt>Entorno</dt><dd>${health.environment}</dd><dt>Smoke test</dt><dd>${canSmoke ? 'Disponible con datos sintéticos' : 'Bloqueado en este rol/entorno'}</dd></div>
    ...
    ctx.root.querySelector<HTMLButtonElement>('#ai-smoke')?.addEventListener('click', async (event) => {
      ...
      await withBusy(button, async () => {
        ...
        output.innerHTML = `<div style="margin-top:12px">${result.ok ? renderAlert('Smoke test correcto.', 'success') : renderAlert('El proveedor respondió, pero no cumplió el contrato sintético.', 'warning')}${renderJson(result)}</div>`;
