...
if (!values.YRAK_STAGING_ACCESS_TEAM_DOMAIN.endsWith('.cloudflareaccess.com')) {
  console.error('YRAK_STAGING_ACCESS_TEAM_DOMAIN must end in .cloudflareaccess.com');
  process.exit(2);
}
...
function stagingName(base, overrideKey) {
  ...
  if (!name.endsWith('-staging')) {
    console.error(`${overrideKey || 'worker name'} must end in -staging: ${name}`);
    process.exit(2);
  }
  return name;
}
...
delete api.vars.BOOTSTRAP_ENABLED;
setD1(api);
if (!api.r2_buckets?.[0]) throw new Error('R2_BINDING_MISSING');
api.r2_buckets[0].bucket_name = values.YRAK_STAGING_R2_BUCKET;
if (!api.workflows?.length) throw new Error('WORKFLOW_BINDING_MISSING');
for (const workflow of api.workflows) workflow.name = values.YRAK_STAGING_WORKFLOW;
if (!api.queues?.producers?.length || !api.queues?.consumers?.length) throw new Error('QUEUE_BINDING_MISSING');
...
setD1(maintenance);
if (!maintenance.queues?.producers?.length) throw new Error('MAINTENANCE_QUEUE_BINDING_MISSING');
