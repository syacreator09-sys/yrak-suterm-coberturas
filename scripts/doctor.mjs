...
const requiredPaths = [
  'pnpm-workspace.yaml', 'turbo.json', 'AGENTS.md', 'CLAUDE.md', '.mcp.json',
  'docs/CLONE_TEST_CONNECT.md', 'docs/CONNECTIONS_CHECKLIST.md', 'docs/RELEASE_CANDIDATE_AUDIT.md',
  'apps/admin-web/package.json', 'apps/api-worker/package.json', 'apps/agent-worker/package.json',
  'apps/mcp-worker/package.json', 'apps/maintenance-worker/package.json', 'apps/employee-portal/package.json',
  'packages/rag/package.json', 'supabase/rag-schema.template.sql',
  'apps/api-worker/wrangler.jsonc', 'apps/agent-worker/wrangler.jsonc', 'apps/mcp-worker/wrangler.jsonc',
  '.claude/skills/yrak-domain-guardrails/SKILL.md', '.claude/skills/yrak-verification/SKILL.md',
  '.claude/skills/yrak-cloudflare-staging/SKILL.md', '.claude/skills/yrak-mcp-readonly/SKILL.md',
  '.claude/skills/yrak-ai-router/SKILL.md', '.claude/skills/yrak-rag-connectors/SKILL.md',
  '.claude/agents/yrak-auditor.md', '.claude/agents/yrak-test-runner.md',
  '.claude/agents/yrak-code-reviewer.md', '.claude/agents/yrak-cloudflare-integrator.md',
  'scripts/check-architecture-boundaries.mjs', 'scripts/check-migrations.mjs', 'scripts/secret-scan.mjs',
  'scripts/render-supabase-rag-schema.mjs', 'scripts/render-staging-configs.mjs', 'scripts/staging-preflight.mjs',
  'scripts/set-staging-bootstrap.mjs', 'scripts/smoke-rag.mjs', 'scripts/smoke-gmail.mjs',
  'scripts/verify-release-candidate.sh', 'scripts/migrate-local.sh',
];
...
if (git.status === 0 && git.stdout.trim() === 'true') {
  ...
  if (branch === 'main') warn('You are on main. Use ready/clone-test-connect-v2 for validation before merging.');
  if (branch && branch !== 'ready/clone-test-connect-v2') warn(`Expected release-candidate branch ready/clone-test-connect-v2; found ${branch}`);
  ...
  const forbiddenTracked = tracked.filter((path) =>
    ...
    path.endsWith('settings.local.json') || path.endsWith('.staging.local.jsonc') ||
...
} else warn('git metadata unavailable; tracked-secret filename check skipped');

const browserFiles = ['apps/admin-web/src', 'apps/admin-web/public', 'apps/admin-web/index.html'];
const serverSecretNames = [
  'OPENAI_API_KEY','ANTHROPIC_API_KEY','AI_COMPAT_API_KEY','RAG_EMBEDDING_API_KEY','HUGGINGFACE_TOKEN',
  'UPSTASH_REDIS_REST_TOKEN','SUPABASE_SERVICE_ROLE','SUPABASE_SERVICE_ROLE_KEY','SUPABASE_SECRET_KEY',
  'MODAL_API_TOKEN','GOOGLE_OAUTH_CLIENT_SECRET','GOOGLE_OAUTH_REFRESH_TOKEN','BOOTSTRAP_TOKEN','MCP_API_TOKEN','AGENT_API_TOKEN',
].join('|');
