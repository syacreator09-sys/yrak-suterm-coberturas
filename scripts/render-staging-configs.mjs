...
if (!/^[0-9a-f-]{20,}$/i.test(values.YRAK_STAGING_D1_DATABASE_ID)) {
  console.error('INVALID_STAGING_VALUE: YRAK_STAGING_D1_DATABASE_ID');
  process.exit(2);
}
if (!/^[a-z0-9][a-z0-9._-]{1,127}$/i.test(values.YRAK_STAGING_D1_DATABASE_NAME)) {
  console.error('INVALID_STAGING_VALUE: YRAK_STAGING_D1_DATABASE_NAME');
  process.exit(2);
}
if (!/^[a-z0-9.-]+\.cloudflareaccess\.com$/i.test(values.YRAK_STAGING_ACCESS_TEAM_DOMAIN)) {
  console.error('INVALID_STAGING_VALUE: YRAK_STAGING_ACCESS_TEAM_DOMAIN must be *.cloudflareaccess.com');
  process.exit(2);
}
if (values.YRAK_STAGING_ACCESS_AUD.length < 8) {
  console.error('INVALID_STAGING_VALUE: YRAK_STAGING_ACCESS_AUD');
  process.exit(2);
}
if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(values.YRAK_STAGING_EMAIL_FROM)) {
  console.error('INVALID_STAGING_VALUE: YRAK_STAGING_EMAIL_FROM');
  process.exit(2);
}
for (const key of ['YRAK_STAGING_R2_BUCKET', 'YRAK_STAGING_QUEUE', 'YRAK_STAGING_WORKFLOW']) {
  if (!/^[a-z0-9][a-z0-9._-]{1,127}$/i.test(values[key])) {
    console.error(`INVALID_STAGING_VALUE: ${key}`);
    process.exit(2);
  }
}
...
function stagingName(base, overrideKey) {
  ...
  const name = override || `${base}-staging`;
  if (!/^[a-z0-9][a-z0-9-]{1,62}$/i.test(name)) throw new Error(`INVALID_WORKER_NAME:${overrideKey}`);
  return name;
}
...
console.log('NEXT: run pnpm preflight:staging before any remote migration/deploy command.');
